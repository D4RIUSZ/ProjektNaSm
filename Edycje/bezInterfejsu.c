...
#define Kp 38.7  // Wzmocnienie proporcjonalne
#define Ki 0.01  // Wzmocnienie całkujące
#define Kd 1.6 // Wzmocnienie różniczkujące
...
void SendDataOverUART(float value, float pidOutput) {
    char buffer[50];
    float n = sprintf(buffer, "Value: %.2f, PID: %.2f\r\n", value, pidOutput);
...
int main(void)
{
  ...
  {
	  nowaw=czujnik();
	  pidprocces=pidController(24, nowaw);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
  ...
  {
	    char buffer[50];
	    int n = sprintf(buffer, "Value: %.2f, PID: %.2f\r\n", value);
	    HAL_UART_Transmit(&huart3, (uint8_t*)buffer, n, 100);
  }
}
/* USER CODE END 4 */
